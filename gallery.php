...
if (isset($_GET['master']) && is_string($_GET['master'])) {
    $masterInput = trim($_GET['master']);
    if (is_gallery_master_key($masterInput)) {
        set_gallery_master_access();
        log_event('gallery_master_login', 'Master-Zugang per URL aktiviert');
        redirect('gallery.php');
    }

    $error = 'Master-Key ungültig.';
    log_event('gallery_login_error', 'Ungültiger Master-Key genutzt');
}
